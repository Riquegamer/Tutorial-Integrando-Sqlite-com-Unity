...
public class Player : MonoBehaviour
{
    ...
    {
        BancoDeDados bancoDeDados = new BancoDeDados();
        bancoDeDados.InserirPosicao(id, transform.position.x, transform.position.y);
    }
    ...
    private void Carregar(int id)
    {
        ...
        if (Leitura != null)
        {
            while (Leitura.Read())
            {
                transform.position = new Vector3(Leitura.GetFloat(1), Leitura.GetFloat(2), transform.position.z);
            }
        }
